...
describe LocalAuthority::UseCase::CreateAccessToken do
  ...
  let(:use_case) { described_class.new(access_token_gateway: access_token_gateway_spy).execute(project_id: 1) }
  ...
  it 'returns different access token' do
    expect(described_class.new(access_token_gateway: access_token_gateway_spy).execute(project_id: 1)).not_to(
      eq(described_class.new(access_token_gateway: access_token_gateway_spy).execute(project_id: 1)))
  end
